...
```{code-cell} ipython3
#On peut rajouter la colonne 'grade' au premier tableau (df1) puis ensuite rajouter les deux lignes manquantes (df3)
```
...
```{code-cell} ipython3
# à vous
df = pd.merge(right = df1, left = df2) #étape 1 : on recolle df1 et df2
df_vf = pd.concat([df, df3]) #étape 2 : on recolle les deux lignes de df3
df_vf
```
...
df = pd.merge(right = df1, left = df2, on = 'name') #étape 1 : on recolle df1 et df2
df_vf = pd.concat([df, df3]) #étape 2 : on recolle les deux lignes de df3
df_vf
```
...
m1['nom complet'] = m1['prenom'] + m1['nom']
m2['nom complet'] = m2['first_name'] + m2['last_name']
m3['nom complet'] = m3['first'] + m3['last']

m1 = m1.set_index('nom complet')
m2 = m2.set_index('nom complet')
m3 = m3.set_index('nom complet')

m = pd.merge(right = m1, left = m2, on = 'nom complet') #étape 1 : on recolle m1 et m2
m_vf = pd.concat([m, m3]) #étape 2 : on recolle les deux lignes de m3
